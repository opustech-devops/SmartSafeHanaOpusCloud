dialog --backtitle "SmartSafeHanaOpusCloud v2.2 - Opus Cloud" --colors --yesno "\Zb\Z4Você deseja que esse backup seja copiado para outro local?" 8 100
...
if [ $COPY_TO_ANOTHER_LOCATION -eq 0 ]; then
    ...
    dialog --backtitle "SmartSafeHanaOpusCloud v2.2 - Opus Cloud" --colors --msgbox "\Zb\Z4O backup não será copiado para outro local." 6 100
fi