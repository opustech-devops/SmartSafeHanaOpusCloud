...
--------------------------------------------------------------------------------
Nota: As senhas são visíveis durante a digitação por limitações do TUI." 28 90 0 \
    ...
    if handle_hdbuserstore "$HDBUSERSTORE_NAME" "$HOSTNAME" "$PORT" "$DATABASE" "$DB_USER" "$DB_PASSWORD" "$USERNAME_LINUX"; then
        ...
        dialog --backtitle "SmartSafeHanaOpusCloud v2.2 - Opus Cloud" --msgbox "Falha na configuração do hdbuserstore para SYSTEMDB. Corrija as credenciais e tente novamente." 7 60
...
for DATABASE in "${DATABASE_ARRAY[@]}"; do
    ...
    exec 3>&1
    VALUES=$(dialog --backtitle "SmartSafeHanaOpusCloud v2.2 - Opus Cloud" --form "Credenciais para Tenant Adicional: $DATABASE" 10 50 0 \
